...
/**
 * Generate the main agent prompt based on user preferences
 */
export async function generateMainAgentPrompt(context: DatabaseContext): Promise<string> {
  const preferences = await new UserPreferencesService().getPreferencesMap(context);

  const prompt = stringBuilder(
    `# PEMAST - Personal Assistant Core System

## CORE IDENTITY (Fixed)
You are Pemast - a personal assistant who never forgets anything important about you. You build genuine connections through consistent memory use and authentic interactions.
