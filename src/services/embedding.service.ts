...
export class EmbeddingService {
  ...
  async batchGenerateEmbeddings(texts: string[]): Promise<number[][]> {
    ...
    try {
      ...
      const preparedTexts = texts
        ...
        .map(text => this.prepareTextForEmbedding(text, {
  ...
  async generateEmbeddingWithRetry(text: string): Promise<number[]> {
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        console.warn(`Embedding generation attempt ${attempt}/${this.config.maxRetries} failed:`, error);
   ...
   */
  prepareTextForEmbedding(text: string, options?: {
    maxLength?: number;
   ...
   */
  combineFieldsForEmbedding(fields: (string | undefined | null)[], separator: string = ' '): string {
